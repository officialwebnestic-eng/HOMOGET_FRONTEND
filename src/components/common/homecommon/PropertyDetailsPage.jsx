...
const PropertyDetailsPage = () => {
  ...
  useEffect(() => {
    const fetchProperty = async () => {
      if (property && property._id === id && typeof property.agentId === 'object') {
        setLoading(false);
        return;
      }
      setLoading(true);
  ...
  const formatPrice = (price, unit) => {
    ...
    let val = num >= 10000000 ? `${(num / 10000000).toFixed(2)} Cr` : num >= 100000 ? `${(num / 100000).toFixed(2)} Lac` : num.toLocaleString("en-IN");
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 ${theme === "dark" ? "bg-[#050505] text-white" : "bg-white text-black"}`}>
      
      {/* 1. ARCHITECTURAL HERO (Visual Only) */}
      <section className="relative w-full h-[75vh] bg-black overflow-hidden">
        <img src={property.image?.[0]} className="w-full h-full object-cover opacity-70 scale-105" alt="Hero" />
        ...
        <div className="max-w-[1440px] mx-auto px-10 py-6 flex flex-wrap justify-between items-center gap-6">
            <div className="flex gap-12">
                <div>
                    ...
                    <p className="text-xl font-bold text-amber-500">{formatPrice(property.price, property.priceUnit)}</p>
                </div>
                <div className="hidden md:block">
                    <p className="text-[8px] uppercase tracking-widest text-zinc-500 font-bold mb-1">Status</p>
                    <p className="text-[11px] font-black uppercase tracking-widest">{property.propertystatus || "Available"}</p>
                </div>
            </div>
            <button 
        ...
        <div className="lg:col-span-8 space-y-32">
            ...
            <section>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-y-12 gap-x-8">
                    ...
                    <SpecItem label="Asset Class" value={property.propertyAssetType} />
                    <SpecItem label="Total Area" value={`${property.squarefoot} Sq.Ft`} />
                    <SpecItem label="Corner Status" value={property.isCorner} />
                    <SpecItem label="District" value={property.city} />
                    <SpecItem label="Road Width" value={property.roadWidth ? `${property.roadWidth} Ft` : "N/A"} />
            ...
            <section className="p-12 border border-zinc-500/10 bg-zinc-500/5 rounded-[2rem]">
                <h3 className="text-[10px] font-black uppercase tracking-[0.5em] text-amber-600 mb-10">Owner Authentication</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
                    <SpecItem label="Legal Owner" value={property.ownerName || "Verified"} />
                    <SpecItem label="Aadhar Verification" value={property.adharVerificationNo || "Secure"} />
                    <div className="space-y-2">
                        <p className="text-[8px] font-bold uppercase opacity-30 tracking-[0.2em]">Title Deed / Khatuni</p>
                        {property.khatuniDocument ? (
                            <a href={property.khatuniDocument} target="_blank" rel="noreferrer" className="text-[10px] flex items-center gap-2 font-black text-amber-500 hover:underline">
                                VIEW DOCUMENT <MoveUpRight size={10} />
                            </a>
                        ) : <p className="text-[10px] text-zinc-500 font-bold">NOT PROVIDED</p>}
                    </div>
                </div>
            </section>
        </div>
        ...
        <aside className="lg:col-span-4">
            <div className="sticky top-32 space-y-12">
                <div className="space-y-8">
                    <h4 className="text-[10px] font-black uppercase tracking-[0.5em] text-amber-500">Representative</h4>
                    <div className="flex items-center gap-6">
                        <div className="w-20 h-20 bg-zinc-900 rounded-full overflow-hidden border border-amber-500/20">
                            <img src={property.agentId?.profilePhoto || ""} className="w-full h-full object-cover grayscale" alt="Agent" />
                        </div>
                        <div>
                            <p className="text-xl font-bold uppercase tracking-tight">{property.agentId?.name}</p>
                            <p className="text-[9px] text-amber-500 font-black uppercase tracking-widest">{property.agentId?.nationality} Expert</p>
                        </div>
                    </div>
                    
                    <div className="space-y-4 pt-6 border-t border-zinc-500/10">
                        <ContactRow icon={<Phone size={14}/>} value={property.agentId?.phone} />
                        <ContactRow icon={<Mail size={14}/>} value={property.agentId?.email} />
                        <ContactRow icon={<Globe size={14}/>} value={`${property.agentId?.city}, ${property.agentId?.state}`} />
                    </div>
                </div>

                <div className="p-8 border border-amber-500/20 bg-amber-500/5 rounded-3xl">
                    <p className="text-[8px] font-black uppercase tracking-widest text-amber-500 mb-2">Verified Agent</p>
        ...
        <section className="px-10 py-24 bg-zinc-500/5">
            <div className="max-w-[1440px] mx-auto grid grid-cols-1 md:grid-cols-2 gap-8">
                {property.image.slice(1, 3).map((img, i) => (
                    <div key={i} className="aspect-video overflow-hidden border border-zinc-500/10">
      ...
      <section className="bg-black py-32">
        <div className="max-w-[1440px] mx-auto px-10 grid grid-cols-1 lg:grid-cols-2 gap-24 items-center">
            <div className="space-y-8">
                <h3 className="text-[10px] font-black uppercase tracking-[0.5em] text-amber-500">Geospatial Position</h3>
                <div className="space-y-2">
                    <p className="text-zinc-500 text-[11px] font-bold uppercase tracking-widest">Address</p>
                    <p className="text-xl font-light italic text-zinc-300">{property.address}</p>
                </div>
                <button 
                ...
                >
                    <Navigation size={14} className="text-amber-500" /> Access Live Maps
                </button>
            </div>
            <div className="relative aspect-video bg-zinc-900 border border-white/5 overflow-hidden group cursor-crosshair">
                <img src="https://images.unsplash.com/photo-1526778548025-fa2f459cd5c1?auto=format&fit=crop&q=80&w=800" className="w-full h-full object-cover grayscale opacity-30 group-hover:opacity-60 transition-opacity" alt="Map" />
...
const SpecItem = ({ label, value, isVerified }) => {
  ...
  return (
    <div className="space-y-1">
      <p className="text-[8px] font-bold uppercase opacity-30 tracking-[0.2em]">{label}</p>
...
const ContactRow = ({ icon, value }) => (
    <div className="flex items-center gap-4 text-zinc-500 hover:text-white transition-colors cursor-pointer group">
        ...
        <span className="text-[11px] font-medium tracking-wide">{value || "Not Disclosed"}</span>
    </div>
);

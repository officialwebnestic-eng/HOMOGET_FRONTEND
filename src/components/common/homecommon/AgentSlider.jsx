...
const AgentSlider = () => {
  ...
  const ct = {
    card: isDark ? "bg-slate-900/40 border-slate-800/60 backdrop-blur-md" : "bg-white border-slate-100 shadow-[0_32px_64px_-12px_rgba(0,0,0,0.08)]",
    text: isDark ? "text-slate-50" : "text-slate-950",
    subText: isDark ? "text-slate-400" : "text-slate-500",
    input: isDark ? "bg-slate-800/40 border-slate-700/50 focus:border-amber-500" : "bg-slate-50 border-slate-200 focus:border-amber-500",
  ...
  const Arrow = ({ onClick, direction }) => (
    ...
    >
      {direction === 'next' ? <ChevronRight size={20} className={direction === 'next' && "group-hover:text-white"} /> : <ChevronLeft size={20} />}
    </motion.button>
  );
  ...
  return (
    <section className={`py-24 px-6 overflow-hidden transition-colors duration-700 ${isDark ? 'bg-slate-950' : 'bg-white'}`}>
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 gap-8">
          <div className="max-w-2xl">
            ...
            >
              ...
              <span className="text-amber-500 text-[10px] font-black uppercase tracking-[0.4em]">The Elite Network</span>
            </motion.div>
            <h2 className={`text-5xl md:text-7xl font-black tracking-tighter leading-none ${ct.text}`}>
              MEET OUR <br/>
              <span className="font-serif italic font-light text-amber-600">Advisors.</span>
            </h2>
          </div>
          <p className={`max-w-xs text-sm font-medium leading-relaxed opacity-70 ${ct.subText}`}>
            Our agents are more than brokers; they are strategic partners in your wealth-building journey.
          </p>
        </div>
                  ...
                  <div className="relative z-10">
                    <div className="flex items-start justify-between mb-8">
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-[10px] font-black uppercase tracking-widest opacity-40">Verified Partner</p>
                      </div>
                    </div>
                    ...
                    <div className="mb-8">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-widest text-amber-600">Senior Property Consultant</span>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-10">
                      ...
                      <div className={`p-4 rounded-2xl border ${isDark ? 'bg-white/5 border-white/5' : 'bg-slate-50 border-slate-100'}`}>
                        <PhoneCall size={14} className="text-amber-500 mb-2" />
                        <p className={`text-[9px] uppercase font-black tracking-tighter ${ct.subText}`}>Available</p>
                      </div>
                    </div>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            <motion.div 
              ...
              onClick={closeModal} className="absolute inset-0 bg-slate-950/90 backdrop-blur-md" 
            />
            <motion.div
              initial={{ opacity: 0, y: 100 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }}
            ...
            >
              ...
              <div className="w-full md:w-2/5 bg-slate-950 p-10 text-white flex flex-col justify-between relative overflow-hidden">
                ...
                <div className="relative z-10">
                   <img src={selectedAgent?.profilePhoto || getAvatarFallback(selectedAgent?.name)} className="w-20 h-20 rounded-2xl mb-6 border border-white/20" alt=""/>
                ...
                <div className="relative z-10 space-y-4">
                   <div className="flex items-center gap-3 opacity-60">
                      <ShieldCheck size={16} />
                      <span className="text-xs">RERA Certified</span>
                   </div>
                   <div className="flex items-center gap-3 opacity-60">
                      <Award size={16} />
                      <span className="text-xs">Top Producer 2025</span>
                   </div>
                </div>
              </div>

              {/* Right Side: Form */}
              <div className="w-full md:w-3/5 p-10 md:p-12 relative">
                ...
                {formSuccess ? (
                  <div className="h-full flex flex-col items-center justify-center text-center space-y-6">
                    ...
                    <p className={`text-sm ${ct.subText}`}>We have established a private channel with {selectedAgent?.name}.</p>
                  </div>
                ) : (
                  <>
                    <div className="mb-8">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                        <input name="firstName" placeholder="First" onChange={(e) => setFormData({...formData, firstName: e.target.value})} required className={`p-4 rounded-xl border outline-none text-xs font-bold ${ct.input}`} />
                        <input name="lastName" placeholder="Last" onChange={(e) => setFormData({...formData, lastName: e.target.value})} required className={`p-4 rounded-xl border outline-none text-xs font-bold ${ct.input}`} />
                      </div>
                      <input name="email" type="email" placeholder="Email Address" onChange={(e) => setFormData({...formData, email: e.target.value})} required className={`w-full p-4 rounded-xl border outline-none text-xs font-bold ${ct.input}`} />
                      <textarea name="message" rows="3" placeholder="Investment requirements..." onChange={(e) => setFormData({...formData, message: e.target.value})} required className={`w-full p-4 rounded-2xl border outline-none text-xs font-bold ${ct.input}`} />

...
export default function Navbar() {
  ...
  const handleOffPlanClick = useCallback(
    (developer = null) => {
      const filters = {
        category: "Off-Plan",
  ...
  /*  MEGA MENU DATA                                                  */
  /* ================================================================ */
  const buyMenuItems = useMemo(
    () => ({
      sections: [
        {
          title: "Residential Sale",
          items: ["Apartments", "Villas", "Townhouses", "Penthouses"],
          action: handleBuyClick,
        },
        {
          title: "Commercial Sale",
          items: ["Office Space", "Retail", "Warehouse", "Land"],
          action: (item) => handleCommercialClick(item, "Sale"),
        },
      ],
      promo: {
      ...
      footer: [
        ...
        },
        {
          ...
          action: () => handleCommercialClick(null, "Sale"),
        },
      ],
    }),
    [handleBuyClick, handleCommercialClick]
  );

  const rentMenuItems = useMemo(
    () => ({
      sections: [
        {
          title: "Residential Rent",
          items: ["Apartments", "Studios", "Villas", "Short Term"],
          action: handleRentClick,
        },
        {
          title: "Commercial Rent",
          items: ["Office Space", "Retail", "Warehouse"],
          action: (item) => handleCommercialClick(item, "Rent"),
        },
      ],
      promo: {
      ...
      footer: [
        ...
        },
        {
          ...
          action: () => handleRentClick("Short Term"),
        },
      ],
    }),
    [handleRentClick, handleCommercialClick, navigate]
  );

  const offplanMenuItems = useMemo(
    () => ({
      sections: [
        {
          title: "New Launches",
          items: ["Emaar Projects", "Damac Hills", "Nakheel", "Sobha Realty"],
          action: (item) => handleOffPlanClick(item.split(" ")[0]),
        },
        {
          title: "By Handover",
          items: ["Handover 2025", "Handover 2026", "Handover 2027"],
          action: (item) =>
            navigateWithFilters({
              category: "Off-Plan",
              deliveryDate: item.split(" ")[1],
            }),
        },
      ],
      promo: {
        ...
        bgClass: "bg-[#f59e0b]",
        action: () => handleOffPlanClick(),
      },
      footer: [
        {
          label: "Project Map",
          icon: <MapPin size={10} />,
          action: () => navigate("/project-map"),
        },
        {
          label: "Payment Plans",
          icon: <DollarSign size={10} />,
          action: () =>
            navigateWithFilters({ category: "Off-Plan", hasPaymentPlan: true }),
        },
      ],
    }),
    [handleOffPlanClick, navigateWithFilters, navigate]
  );
  ...
  /* ================================================================ */
  /*  LOGOUT — FIXED: guards against double-click, always navigates    */
  /* ================================================================ */
  const handleLogout = useCallback(async () => {
    ...
    try {
      ...
      // Always navigate, even if API failed
      navigate("/login", { replace: true });
    }
  }, [isLoggingOut, logoutUser, navigate]);

  const toggleMobileDropdown = useCallback((menuName) => {
    setActiveMobileDropdown((prev) => (prev === menuName ? null : menuName));
  }, []);
  ...
  const renderBadge = useCallback((type) => {
    ...
    return (
      <span
        className={`ml-1 px-1 py-0.5 rounded text-[6px] font-black text-white uppercase ${colors[type]}`}
      >
        {type}
      </span>
    );
  ...
  // ⚡ FIXED: text color now uses dark on light + proper dark mode
  const navLinkClass =
    "px-3 py-2 text-[10px] font-bold uppercase tracking-wider text-slate-800 dark:text-slate-200 hover:text-amber-500 transition-colors whitespace-nowrap flex items-center gap-1 cursor-pointer";
  ...
  const MobileDropdown = ({ title, isOpen, onToggle, children }) => (
    <div className="border-b border-slate-100 dark:border-white/10">
      <button
        ...
        className="w-full flex items-center justify-between py-3 text-[11px] font-bold uppercase tracking-wider text-slate-800 dark:text-slate-200"
      >
        {title}
        <ChevronDownIcon
    ...
    <>
      <nav
        className={`sticky top-0 z-[100] backdrop-blur-xl ${
          isDark
            ? "bg-[#0a0a0c]/95 text-white"
            : "bg-white/95 text-slate-900"
        } border-b ${isDark ? "border-white/5" : "border-slate-100"}`}
      >
          ...
          <div className="flex items-center justify-between h-14 lg:h-16 gap-4">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              {showSidebarToggle && (
                <button
                  ...
                  className="p-1.5 rounded-lg hover:bg-[#f59e0b] text-black transition-all"
                >
                  <MenuLucide size={16} />
                </button>
              )}
                ...
                {navItems.map((item) => {
                  ...
                  if (item === "About Us" || item === "Contact Us") {
                    return (
                      <Link
                        key={item}
                        to={item === "About Us" ? "/about-us" : "/contact-us"}
                  ...
                  if (item === "Blogs") {
                    return (
                      <Link key={item} to="/blog" className={navLinkClass}>
                        {item}
                        <span className="bg-amber-500 text-black text-[6px] px-1.5 py-0.5 rounded-full font-bold ml-1">
                          NEW
                        </span>
                      </Link>
                    );
                    ...
                    return (
                      ...
                      >
                        <button
                          type="button"
                          className={`${navLinkClass} ${
                            activeMegaMenu === "Buy" ? "text-amber-500" : ""
                          }`}
                        >
                          Buy{" "}
                          <ChevronDownIcon
                            className={`h-2.5 w-2.5 transition-transform ${
                              activeMegaMenu === "Buy" ? "rotate-180" : ""
                            }`}
                          />
                        </button>
                          ...
                          <div className="p-4 flex gap-4">
                            <div className="flex-1 grid grid-cols-2 gap-4">
                              {buyMenuItems.sections.map((section, idx) => (
                                <div key={idx}>
                                  <h4 className="text-[9px] font-bold text-amber-500 uppercase mb-2 tracking-wider">
                                    {section.title}
                                  </h4>
                                  <div className="space-y-1">
                                    {section.items.map((subItem) => (
                                      <button
                                        key={subItem}
                                        type="button"
                                        onClick={() => section.action(subItem)}
                                        className="block text-[11px] font-medium text-slate-700 dark:text-slate-300 hover:text-amber-500 transition-colors py-1"
                                      >
                                        {subItem}
                                      </button>
                                    ))}
                            ...
                            <button
                              type="button"
                              onClick={buyMenuItems.promo.action}
                              className="w-36 rounded-lg p-3 bg-[#2D2D6E] text-white flex flex-col justify-end shadow-lg hover:scale-105 transition-transform"
                            >
                              <p className="text-[7px] font-bold uppercase opacity-60">
                                {buyMenuItems.promo.subtitle}
                              </p>
                          ...
                          <div className="bg-gray-50 dark:bg-white/5 p-3 flex gap-2 border-t">
                            {buyMenuItems.footer.map((footerItem, idx) => (
                              <button
                                key={idx}
                                type="button"
                                onClick={footerItem.action}
                                className="px-3 py-1.5 rounded-lg text-[8px] font-bold uppercase flex items-center gap-1.5 bg-white dark:bg-white/10 border border-slate-200 dark:border-white/10 hover:border-amber-500 hover:text-amber-500 transition-all"
                              >
                    ...
                    return (
                      ...
                      >
                        <button
                          type="button"
                          className={`${navLinkClass} ${
                            activeMegaMenu === "Rent" ? "text-amber-500" : ""
                          }`}
                        >
                          Rent{" "}
                          <ChevronDownIcon
                            className={`h-2.5 w-2.5 transition-transform ${
                              activeMegaMenu === "Rent" ? "rotate-180" : ""
                            }`}
                          />
                        </button>
                        <DesktopDropdown isOpen={activeMegaMenu === "Rent"}>
                          <div className="p-4 flex gap-4">
                            ...
                            <button
                              type="button"
                              onClick={rentMenuItems.promo.action}
                              className="w-36 rounded-lg p-3 bg-[#5D46A0] text-white flex flex-col justify-end shadow-lg hover:scale-105 transition-transform"
                            >
                              <p className="text-[7px] font-bold uppercase opacity-60">
                                {rentMenuItems.promo.subtitle}
                              </p>
                              <p className="text-[11px] font-bold leading-tight mt-0.5">
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          ...
                          <ChevronDownIcon
                            className={`h-2.5 w-2.5 transition-transform ${
                              activeMegaMenu === "Off-Plan" ? "rotate-180" : ""
                            }`}
                          />
                        </button>
                        <DesktopDropdown isOpen={activeMegaMenu === "Off-Plan"}>
                          <div className="p-4 flex gap-4">
                            ...
                            <button
                              type="button"
                              onClick={offplanMenuItems.promo.action}
                              className="w-36 rounded-lg p-3 bg-[#f59e0b] text-black flex flex-col justify-end shadow-lg hover:scale-105 transition-transform"
                            >
                              <p className="text-[7px] font-bold uppercase opacity-60">
                                {offplanMenuItems.promo.subtitle}
                              </p>
                              <p className="text-[11px] font-bold leading-tight mt-0.5">
                        ...
                        >
                          {sidebarLinks.map((link) => (
                            <Link
                              key={link.name}
                              to={link.href}
                              className="flex items-center justify-between px-3 py-1.5 rounded-lg text-[9px] font-bold uppercase tracking-wider text-slate-700 dark:text-slate-300 hover:bg-amber-500 hover:text-white transition-all"
            ...
            <div className="flex items-center gap-1 sm:gap-2 lg:gap-2 flex-shrink-0">
              ...
              {!isAuthenticated ? (
                <Link
                  to="/login"
                  className="px-3 sm:px-4 py-1.5 text-[8px] sm:text-[9px] font-black uppercase tracking-wider rounded-full bg-amber-500 text-white shadow-md hover:bg-amber-600 transition-all flex-shrink-0 whitespace-nowrap"
                >
                  Login
                </Link>
              ) : (
                        ...
                        <div className="p-1">
                          {user?.role === "user" && (
                            <>
                              <Link
                                to="/user-profile"
                                className="flex items-center gap-2 px-3 py-1.5 rounded-lg text-[9px] font-bold text-slate-700 dark:text-slate-200 hover:bg-amber-500/10 transition-all"
                          ...
                          <button
                            ...
                            className="w-full flex items-center gap-2 px-3 py-1.5 rounded-lg text-[9px] font-bold text-red-500 hover:bg-red-500/10 transition-all disabled:opacity-60"
                          >
                            <LogOut size={12} />
              ...
              <button
                ...
                className="lg:hidden p-1.5 text-slate-900 dark:text-white flex-shrink-0"
              >
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-1">
              <Link
                to="/"
                onClick={() => setIsMobileMenuOpen(false)}
                className="block py-3 text-[11px] font-bold uppercase tracking-wider text-slate-900 dark:text-white border-b border-slate-100 dark:border-white/10"
              >
                Home
              </Link>
              ...
              >
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <p className="text-[9px] font-bold text-amber-500 uppercase mb-2">
                        Residential Sale
                      </p>
                      <div className="space-y-1">
                        {buyMenuItems.sections[0].items.map((item) => (
                          <button
                            ...
                            className="block text-[10px] font-medium py-1 hover:text-amber-500 w-full text-left text-slate-700 dark:text-slate-300"
                          >
                            {item}
                          </button>
                        ))}
                  ...
                  <button
                    type="button"
                    onClick={buyMenuItems.promo.action}
                    className="mt-2 w-full py-2 rounded-lg bg-[#2D2D6E] text-white text-[9px] font-bold uppercase"
                  >
                    {buyMenuItems.promo.title}
                  </button>
                </div>
              ...
              >
                <div className="space-y-4">
                  ...
                  <button
                    type="button"
                    onClick={rentMenuItems.promo.action}
                    className="mt-2 w-full py-2 rounded-lg bg-[#5D46A0] text-white text-[9px] font-bold uppercase"
                  >
                    {rentMenuItems.promo.title}
                  </button>
                </div>
              ...
              >
                <div className="space-y-4">
                      ...
                      <div className="space-y-1">
                        {offplanMenuItems.sections[0].items.map((item) => (
                          <button
                            ...
                            type="button"
                            onClick={() =>
                              handleOffPlanClick(item.split(" ")[0])
                            }
                            className="block text-[10px] font-medium py-1 hover:text-amber-500 w-full text-left text-slate-700 dark:text-slate-300"
                          >
                            {item}
                  ...
                  <button
                    type="button"
                    onClick={offplanMenuItems.promo.action}
                    className="mt-2 w-full py-2 rounded-lg bg-[#f59e0b] text-black text-[9px] font-bold uppercase"
                  >
                    {offplanMenuItems.promo.title}
                  </button>
                </div>
                ...
                <div className="grid grid-cols-1 gap-1 pl-2">
                  {sidebarLinks.map((link) => (
                    <Link
                      ...
                      to={link.href}
                      onClick={() => setIsMobileMenuOpen(false)}
                      className="flex items-center justify-between py-2 text-[10px] font-medium text-slate-700 dark:text-slate-300 hover:text-amber-500"
                    >
                      {link.name} {renderBadge(link.badge)}
                    </Link>
                  ))}
              ...
              >
                Careers{" "}
                <span className="ml-1 bg-amber-500 text-black text-[6px] px-1.5 py-0.5 rounded-full font-bold">
                  HIRING
                </span>
              </Link>
            </div>
